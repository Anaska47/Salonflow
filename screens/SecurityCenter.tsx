...
const SecurityCenter = () => {
  ...
  const handleExport = () => {
    errorLogService.log('info', "Export de la base de données initié", "Admin", user?.name);
    db.exportDatabase();
  };
  ...
  const handleImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      ...
      if (window.confirm("ALERTE : L'importation va ÉCRASER toutes vos données actuelles. Continuer ?")) {
        await db.importDatabase(content);
        errorLogService.log('critical', "Restauration de base de données effectuée", "Migration JSON", user?.name);
      }
    };
  ...
  const handleNuclearWipe = async () => {
    ...
    errorLogService.log('critical', "DESTRUCTION TOTALE DE LA BASE", "Action Owner", user?.name);
          ...
          {isOwner && (
            <div className="bg-white rounded-[3rem] p-8 border border-slate-200 shadow-sm space-y-8">
               <div className="flex justify-between items-center">
                  ...
                  <div className="px-4 py-2 bg-indigo-50 text-indigo-600 rounded-xl text-[10px] font-black uppercase">MockDB v1.0</div>
               </div>
